...
plugin_ctrl: voodoo=false, pcidev=false, e1000=false, gameport=true, usb_ehci=false, speaker=true, es1370=false, extfpuirq=true, serial=true, usb_ohci=false, iodebug=true, sb16=false, busmouse=false, biosdev=true, usb_uhci=false, usb_xhci=false, unmapped=true, parallel=true, pcipnic=false, ne2k=false
...
- `display_library: x, options="gui_debug"`：打开图形调试模式。
- `romimage: file="/usr/share/bochs/BIOS-bochs-legacy", address=0x00000000, options=none`：替换为这一行，否则会报错。
- `boot: disk`：从硬盘启动。
- `ata0-master: type=disk, path="../build/master.img", mode=flat`：指定硬盘镜像文件。
- `magic_break: enabled=1`：打开 bochs 魔术断点。
